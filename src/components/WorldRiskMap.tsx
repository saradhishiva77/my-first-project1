...
// Map TopoJSON country names to our CountryInfo names
...
export function WorldRiskMap({ onSelect }: { onSelect?: (view: CountryRiskView) => void }) {
  ...
  const { paths, centroids } = useMemo(() => {
    ...
    const paths = features.map((f) => {
      ...
      if (countryInfo) {
        centroids[countryName] = [centroid[0], centroid[1]];
      }
  ...
  return (
    <div className="relative w-full">
      ...
      <svg viewBox="0 0 1000 500" className="w-full h-full" style={{ minHeight: 480 }}>
        ...
        {paths.map((p, i) => {
          if (!p.countryInfo) {
            // Still render countries we don't have data for, in a neutral color
            return (
              <path
                key={i}
                d={p.d}
          ...
          return (
            <g key={i}>
              <path
                d={p.d}
                fill={color}
                fillOpacity={dimmed ? 0.12 : isSelected ? 0.85 : isHover ? 0.7 : 0.5}
                stroke={isSelected ? '#fff' : isHover ? 'rgba(255,255,255,0.6)' : 'rgba(7,11,20,0.5)'}
                strokeWidth={isSelected ? 1.5 : isHover ? 1 : 0.5}
                strokeLinejoin="round"
                filter={isSelected ? 'url(#countryGlowReal)' : undefined}
                className="cursor-pointer transition-all duration-200"
                onMouseEnter={() => setHover(p.countryName)}
              ...
              />
              {/* Pulsing dot for critical countries */}
